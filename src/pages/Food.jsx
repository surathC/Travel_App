        ...
        const applyFilters = () => {
            ...
            const filtered = foods.filter((food) => {
                return mainFilters.every((filter) => {
                    ...
                    if (filter === "District") {
                        return selectedOptions[filter].includes(food.district?.name);
                    }
                    if (filter === "City") {
                        return selectedOptions[filter].includes(food.foodCities?.name);
                    }
                    if (filter === "Main Category") {
                        return food.categories?.some((category) =>
        ...
        if (searchTerm.trim() === "") {
            ...
            const filtered = foods.filter((food) => {
                return (
                    ...
                    food.foodCode.toLowerCase().includes(searchLower) ||
                    (food.district?.name && food.district.name.toLowerCase().includes(searchLower)) ||
                    food.categories.some((category) => category.name.toLowerCase().includes(searchLower)) ||
                    food.foodCities.some((city) => city.name.toLowerCase().includes(searchLower))
                );
            });
                        ...
                        >
                            ...
                            <div className={`w-full ${isVerticalLayout ? 'p-4' : 'sm:w-2/3 sm:p-4 md:p-8'}`}>
                                    ...
                                    <div className="flex mt-2">
                                        {Array.from({ length: 5 }, (_, index) => (
                                            <svg
                                                key={index}
                                                xmlns="http://www.w3.org/2000/svg"
                                                fill={index < food.rating ? "yellow" : "none"}
                                                stroke="currentColor"
                                ...
                                <div className="flex flex-col sm:flex-row items-center gap-4">
                                    ...
                                    <Link to='/foodDetails' state={{ food }}>
                                        <button className="bg-blue-500 text-white px-4 py-2 text-sm rounded-full hover:bg-blue-600">
                                            More details
                                        </button>
                                    </Link>
                                </div>
                                <p className="text-gray-500 mt-5 text-xs sm:text-sm">{food.updatedOn}</p>
                            </div>
                        </div>
                    ))}
                </div>
